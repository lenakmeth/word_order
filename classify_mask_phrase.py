...
logger = open('logs/frwac_jan/mask_phrase/' + args.transformer_model.split('/')[-1] + '.log', 'a+')
        ...
        try:
            ...
            if len(tok_noun) == 3 and len(tok_adj) == 3:
                ...
                if tok_noun[1] in encoded_dict['input_ids'] and \
                    ...
                    attn_mask = [1] * len(encoded_dict['input_ids'])
                    attn_mask[encoded_dict['input_ids'].index(tok_noun[1])] = 0
                    attn_mask[encoded_dict['input_ids'].index(tok_adj[1])] = 0
...
train_inputs, train_masks, train_segments, train_labels, train_sentences = tokenize_and_mask(train_sentences)
val_inputs, val_masks, val_segments, val_labels, val_sentences = tokenize_and_mask(val_sentences)
test_inputs, test_masks, test_segments, test_labels, test_sentences = tokenize_and_mask(test_sentences)
ud_inputs, ud_masks, ud_segments, ud_labels, ud_test = tokenize_and_mask(ud_test)
